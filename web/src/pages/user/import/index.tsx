...
interface ImportPageCache {
  ...
  /// Persist scroll positions across navigate-away → navigate-back. Without
  /// this, the new DOM after remount has scrollTop=0 on both panes and the
  /// user sees "left jumped to top" (and dst would too, but dst's longer
  /// content makes the reset less visually obvious). Restored synchronously
  /// in useLayoutEffect on mount; see savedSourceScroll / savedDraftsScroll.
  srcScrollTop: number;
  dstScrollTop: number;
...
// Debug accessor for chrome MCP / DevTools to inspect cache contents.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
if (typeof window !== 'undefined') (window as any).__getImportPageCache = () => importPageCache;
...
export default function UserBulkImportPage() {
  ...
  // 2026-05-07: initial value seeded from importPageCache so post-remount
  // restore brings the saved scroll back instead of clobbering to 0.
  const sourceScrollRef = useRef<HTMLDivElement | null>(null);
  const textareaRef = useRef<HTMLTextAreaElement | null>(null);
  const savedSourceScroll = useRef<number>(hydrate?.srcScrollTop ?? 0);
  const savedDraftsScroll = useRef<number>(hydrate?.dstScrollTop ?? 0);
  // 2026-05-07: only run the heavy "restore from cache + 600ms follower window
  // + polling" path on the FIRST useLayoutEffect run after mount. Subsequent
  // re-runs (sourceEditMode toggles, or React-induced spurious re-runs from
  // sibling state changes / TanStack Query refetches) must not re-trigger the
  // restore — that was clobbering the user's mid-scroll position back to the
  // hydrate values and locking sync for 600ms, producing visible "src jumps
  // to top" mid-scroll. Logs captured this exactly: sync write target=0
  // landed right after init-bail-restore-window with untilMs≈583.
  ...
  // 2026-05-07: also restore drafts pane scroll so navigate-away→back keeps
  // both panes at their previous positions.
  useLayoutEffect(() => {
    if (state !== 'working') return;
    // Skip if we've already restored on this mount. Subsequent re-runs of
    // this effect (sourceEditMode toggles, or React-induced spurious re-runs
    // when `state` reference changes despite same value, or sibling state
    // cascades) must NOT re-trigger the heavy restore — that was clobbering
    // the user's mid-scroll position back to hydrate values and locking
    // sync for 600ms. Bug evidence: logs captured `init-bail-restore-window`
    // appearing mid-scroll with untilMs≈583, immediately followed by sync
    // write target=0 (src jumped to top).
    if (restoreDoneRef.current) {
      // For sourceEditMode toggles, just sync the current scroll between
      // outer div and textarea (the original purpose of this effect).
      if (sourceEditMode && textareaRef.current) {
        textareaRef.current.scrollTop = savedSourceScroll.current;
        textareaRef.current.focus({ preventScroll: true });
      } else if (!sourceEditMode && sourceScrollRef.current) {
        sourceScrollRef.current.scrollTop = savedSourceScroll.current;
      }
      return;
    }
    restoreDoneRef.current = true;

    const savedSrc = savedSourceScroll.current;
    const savedDst = savedDraftsScroll.current;
    const sc = sourceScrollRef.current;
    const ta = textareaRef.current;
    const dc = draftsScrollRef.current;

    // 2026-05-07 (data-driven fix v3): restore scrollTop with polling until
    // pane scrollHeight is large enough to actually accommodate the saved
    // value. Without polling, useLayoutEffect (and even its 1-frame rAF
    // retry) can fire BEFORE all 15 draft cards have laid out fully — at
    // that moment dst.scrollHeight might be only ~775px (1-2 cards visible),
    // and `dc.scrollTop = 2463` gets clamped by the browser to ~258. Once
    // cards finish laying out scrollHeight grows but scrollTop stays at the
    // clamped value (browsers don't re-apply pending scrollTop). User's
    // probe captured exactly this: cache.dst=2463, actual.dst=258.5.
    //
    // Strategy: re-check scrollHeight every frame until it can fit savedDst,
    // then apply both panes atomically. Cap at ~33 frames (~550ms) so we
    // don't loop forever if content actually shrunk between save & restore.
    const RESTORE_FOLLOWER_MS = 600;
    const restoreUntil = Date.now() + RESTORE_FOLLOWER_MS;
    followerUntilRef.current.src = restoreUntil;
    followerUntilRef.current.dst = restoreUntil;
    ...
    const tryApply = (attempt: number) => {
      if (cancelled) return;
      // Refresh follower window — every retry frame should keep sync bailed.
      followerUntilRef.current.src = Math.max(followerUntilRef.current.src, Date.now() + 200);
      followerUntilRef.current.dst = Math.max(followerUntilRef.current.dst, Date.now() + 200);

      const dstReady = !dc || (dc.scrollHeight - dc.clientHeight) >= savedDst - 1;
      const srcReady = !sc || (sc.scrollHeight - sc.clientHeight) >= savedSrc - 1;
      const exhausted = attempt >= 33;  // ~550ms cap

      if (dstReady && srcReady) {
        if (sc) { /* read forces layout */ void sc.scrollHeight; sc.scrollTop = savedSrc; }
        if (ta && sourceEditMode) { void ta.scrollHeight; ta.scrollTop = savedSrc; }
        if (dc) { void dc.scrollHeight; dc.scrollTop = savedDst; }
        return;
      }
      if (exhausted) {
        // Layout never grew enough — accept best-effort (browser clamps).
        if (sc) sc.scrollTop = savedSrc;
        if (ta && sourceEditMode) ta.scrollTop = savedSrc;
        if (dc) dc.scrollTop = savedDst;
        return;
      }
      rafId = requestAnimationFrame(() => tryApply(attempt + 1));
    };
  ...
  // 2026-05-07 v16 — bidirectional scroll-sync, stateless screen-coordinate
  // alignment. Each invocation finds the leader-side element whose
  // bounding-rect is closest to the leader's viewport center, locates the
  // bound element on the follower side, and adjusts the follower's
  // scrollTop so the bound element lands at the same screen-Y. No pair
  // list, no ext ranges, no neighbor-dependent math — the previous v11~v15
  // pixel-mapping + gap-absorption approach was vulnerable to mid-scroll
  // layout flicker (sibling re-renders shifting card offsetTops between
  // sync frames produced target=0 jumps). See git history for the older
  // gap-absorption approach if a regression makes a revisit useful.
  useEffect(() => {
    ...
    // Short follower window applied right before writing the OTHER side, so
    // the scroll event our write generates can't trigger a reverse sync.
    // 80ms covers the rAF + scroll-event round-trip with margin; well under
    // INERTIA_MS so a real user gesture on the other side still wins.
    const SYNC_WRITE_FOLLOWER_MS = 80;

    // Debug: console-log every sync decision when window.AIKEY_SYNC_DEBUG === true.
    // Toggle in DevTools: window.AIKEY_SYNC_DEBUG = true / false.
    // Each line tagged [sync] for easy grep, with action (write/skip/bail-*).
    function dbg(direction: 'src→dst' | 'dst→src', action: string, info: Record<string, unknown>) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (!(window as any).AIKEY_SYNC_DEBUG) return;
      // eslint-disable-next-line no-console
      console.log(`[sync ${direction}] ${action}`, info);
    }

    // 2026-05-07 v16: stateless screen-coordinate sync.
    //
    // Replaces the pixel-mapping algorithm (buildSyncPairs + ext-range +
    // followerScrollFromLeader) which was vulnerable to layout-flicker
    // glitches: it reads offsetTop of all 15 cards/lines on each sync
    // invocation, computes ext ranges from neighbor positions, and maps
    // focal pixel to follower pixel. Mid-scroll React re-renders (e.g.,
    // TanStack Query refetches updating a chart) shifted card offsetTops
    // transiently between sync frames; algorithm produced target=0 from
    // boundary-clamp paths, snapping src to top.
    //
    // New mechanism — element-anchored, screen-coordinate, self-contained:
    //   1. Find the leader pane's screen-Y focal point (viewport center).
    //   2. Find the visible CARD/LINE element closest to that screen-Y on
    //      leader (uses getBoundingClientRect, screen-relative & always live).
    //   3. Find the BOUND element on the follower side (cardEl ↔ first source
    //      line of that draft's line_range).
    //   4. Compute scroll adjustment so the follower's bound element lands
    //      at the same screen Y as the leader's anchor.
    //   5. Apply: follower.scrollTop += adjustment.
    //
    // Why this is robust against layout flicker:
    //   - getBoundingClientRect is screen-relative, always reflects current
    //     post-layout state. No reading of multiple offsetTops with
    //     between-frame inconsistency.
    //   - Each sync invocation is self-contained (no pair list, no ext
    //     ranges, no neighbor-dependent math, no accumulator state).
    //   - If no card visible (DOM mid-render) → return without writing →
    //     follower stays put → no jump-to-top failure mode.
    //   - When a card is visible, we land the bound source line at the
    //     same screen Y. Algorithm has no concept of "first pair / last
    //     pair / clamp to 0" — those clamping paths were the source of
    //     target=0 glitches.
    function findAnchorOnLeader(
      leader: HTMLElement, leaderSelector: string,
    ): { el: HTMLElement; screenY: number } | null {
      const leaderRect = leader.getBoundingClientRect();
      const focalScreenY = leaderRect.top + leader.clientHeight / 2;
      const elements = leader.querySelectorAll<HTMLElement>(leaderSelector);
      if (elements.length === 0) return null;
      let best: HTMLElement | null = null;
      let bestDist = Infinity;
      for (const el of elements) {
        const r = el.getBoundingClientRect();
        // Distance from focalScreenY to the element's vertical span
        const dist = focalScreenY < r.top
          ? r.top - focalScreenY
          : focalScreenY > r.bottom
            ? focalScreenY - r.bottom
            : 0;
        if (dist < bestDist) {
          bestDist = dist;
      ...
      if (!best) return null;
      // Return the screen Y where we want the bound follower element to land —
      // matches focalScreenY (we always align bound-element top to focal).
      return { el: best, screenY: focalScreenY };
    }

    function syncFromSource() {
      if (!src || !dst) return;
      if (isFollower('src')) {
        dbg('src→dst', 'bail-src-is-follower', { srcTop: src.scrollTop, dstTop: dst.scrollTop, untilMs: followerUntilRef.current.src - Date.now() });
        return;
      }
      if (Date.now() < clickJumpUntilRef.current) {
        dbg('src→dst', 'bail-clickjump-cooldown', { remainingMs: clickJumpUntilRef.current - Date.now() });
        return;
      }
      // Edge clamp: when leader is at an extreme, push follower to the matching
      // extreme. The center-anchor algorithm otherwise leaves upper/lower cards
      // off-viewport (e.g., src.scrollTop=0 anchors line ~12 at src center,
      // putting d-4 at dst center — d-1/d-2/d-3 cards become invisible above
      // dst's viewport). User-reported "右侧没有彻底到顶部，一张卡片看不到".
      const srcMax = Math.max(0, src.scrollHeight - src.clientHeight);
      const dstMax = Math.max(0, dst.scrollHeight - dst.clientHeight);
      if (src.scrollTop <= 2 && dst.scrollTop !== 0) {
        followerUntilRef.current.dst = Date.now() + SYNC_WRITE_FOLLOWER_MS;
        dst.scrollTop = 0;
        dbg('src→dst', 'edge-clamp-top', { srcTop: src.scrollTop });
        return;
      }
      if (src.scrollTop >= srcMax - 2 && dst.scrollTop !== dstMax) {
        followerUntilRef.current.dst = Date.now() + SYNC_WRITE_FOLLOWER_MS;
        dst.scrollTop = dstMax;
        dbg('src→dst', 'edge-clamp-bottom', { srcTop: src.scrollTop, srcMax });
        return;
      }
      // src is leader: find the source line closest to viewport center,
      // then map it to its bound draft card.
      const anchor = findAnchorOnLeader(src, '[data-line]');
      if (!anchor) {
        dbg('src→dst', 'bail-no-anchor', { srcTop: src.scrollTop });
        return;
      }
      const lineNum = anchor.el.getAttribute('data-line');
      if (lineNum == null) return;
      const lineNumInt = parseInt(lineNum, 10);
      // Find draft whose line_range contains lineNumInt
      const draft = drafts.find((d) => {
        const [s, e] = d.record.line_range;
        return lineNumInt >= s && lineNumInt <= e;
      });
      if (!draft) {
        dbg('src→dst', 'bail-line-no-draft', { lineNum });
        return;
      }
      const cardEl = dst.querySelector<HTMLElement>(`[data-draft-id="${draft.record.id}"]`);
      if (!cardEl) {
        dbg('src→dst', 'bail-card-missing', { draftId: draft.record.id });
        return;
      }
      const cardRect = cardEl.getBoundingClientRect();
      const adjustment = cardRect.top - anchor.screenY;
      const newDstScrollTop = dst.scrollTop + adjustment;
      if (Math.abs(adjustment) < 2) {
        dbg('src→dst', 'skip-already-aligned', { srcTop: src.scrollTop, dstTop: dst.scrollTop, adjustment });
        return;
      }
      dbg('src→dst', 'write', { srcTop: src.scrollTop, dstTop: dst.scrollTop, newDstScrollTop, adjustment, anchorLine: lineNum, draftId: draft.record.id });
      followerUntilRef.current.dst = Date.now() + SYNC_WRITE_FOLLOWER_MS;
      dst.scrollTop = newDstScrollTop;
    }

    function syncFromDrafts() {
      if (!src || !dst) return;
      if (isFollower('dst')) {
        dbg('dst→src', 'bail-dst-is-follower', { srcTop: src.scrollTop, dstTop: dst.scrollTop, untilMs: followerUntilRef.current.dst - Date.now() });
        return;
      }
      if (Date.now() < clickJumpUntilRef.current) {
        dbg('dst→src', 'bail-clickjump-cooldown', { remainingMs: clickJumpUntilRef.current - Date.now() });
        return;
      }
      // Edge clamp (mirror of syncFromSource — see comment there).
      const srcMax = Math.max(0, src.scrollHeight - src.clientHeight);
      const dstMax = Math.max(0, dst.scrollHeight - dst.clientHeight);
      if (dst.scrollTop <= 2 && src.scrollTop !== 0) {
        followerUntilRef.current.src = Date.now() + SYNC_WRITE_FOLLOWER_MS;
        src.scrollTop = 0;
        dbg('dst→src', 'edge-clamp-top', { dstTop: dst.scrollTop });
        return;
      }
      if (dst.scrollTop >= dstMax - 2 && src.scrollTop !== srcMax) {
        followerUntilRef.current.src = Date.now() + SYNC_WRITE_FOLLOWER_MS;
        src.scrollTop = srcMax;
        dbg('dst→src', 'edge-clamp-bottom', { dstTop: dst.scrollTop, dstMax });
        return;
      }
      // dst is leader: find the draft card closest to viewport center,
      // then map it to its bound source line (line_range[0]).
      const anchor = findAnchorOnLeader(dst, '[data-draft-id]');
      if (!anchor) {
        dbg('dst→src', 'bail-no-anchor', { dstTop: dst.scrollTop });
        return;
      }
      const draftId = anchor.el.getAttribute('data-draft-id');
      if (!draftId) return;
      const draft = drafts.find((d) => d.record.id === draftId);
      if (!draft) {
        dbg('dst→src', 'bail-draft-missing', { draftId });
        return;
      }
      const lineEl = src.querySelector<HTMLElement>(`[data-line="${draft.record.line_range[0]}"]`);
      if (!lineEl) {
        dbg('dst→src', 'bail-line-missing', { line: draft.record.line_range[0] });
        return;
      }
      const lineRect = lineEl.getBoundingClientRect();
      const adjustment = lineRect.top - anchor.screenY;
      const newSrcScrollTop = src.scrollTop + adjustment;
      if (Math.abs(adjustment) < 2) {
        dbg('dst→src', 'skip-already-aligned', { srcTop: src.scrollTop, dstTop: dst.scrollTop, adjustment });
        return;
      }
      dbg('dst→src', 'write', { srcTop: src.scrollTop, dstTop: dst.scrollTop, newSrcScrollTop, adjustment, draftId, anchorLine: draft.record.line_range[0] });
      followerUntilRef.current.src = Date.now() + SYNC_WRITE_FOLLOWER_MS;
      src.scrollTop = newSrcScrollTop;
    }
    ...
    let rafDst = 0;
    const onSrcScroll = () => {
      // Track latest src scrollTop so a future useLayoutEffect re-run (e.g.,
      // sourceEditMode toggle, state change) restores to the LATEST value,
      // not the stale hydrate value.
      if (!sourceEditMode) savedSourceScroll.current = src.scrollTop;
      cancelAnimationFrame(rafSrc);
      rafSrc = requestAnimationFrame(syncFromSource);
    };
    const onDstScroll = () => {
      // 2026-05-07: track dst scroll into savedDraftsScroll. Without this,
      // savedDraftsScroll stays at hydrate value forever; if useLayoutEffect
      // re-runs later (sourceEditMode/state change), it restores dst to the
      // stale hydrate value and the user's current scroll position is lost.
      savedDraftsScroll.current = dst.scrollTop;
    ...
    const onSrcGesture = (e: Event) => {
      markUserGesture('src');
      dbg('src→dst', `gesture:${e.type}`, { srcTop: src.scrollTop, dstTop: dst.scrollTop });
    };
    const onDstGesture = (e: Event) => {
      markUserGesture('dst');
      dbg('dst→src', `gesture:${e.type}`, { srcTop: src.scrollTop, dstTop: dst.scrollTop });
    };
    ...
    // 2026-05-07: previously had a `reSyncOnFocus` handler that on window
    // focus / visibility regain forcibly recomputed src from dst. Removed
    // because: (a) sync handlers continue running while window is defocused
    // (scroll events fire regardless), so panes should already be in sync
    // when user refocuses; (b) the forced recompute changed the user's
    // visible scroll position post-focus, which they reported as a bug.
  ...
  // 2026-05-07: also persist scrollTop on each cache write so post-remount
  // hydration restores the user's scroll position. The savedSourceScroll /
  // draftsScrollRef refs hold the latest values; cache always picks them up
  // when state/drafts change. On unmount (useEffect cleanup) we do one more
  // sync to capture the very latest scroll positions in case the user just
  // scrolled and didn't trigger another React state change.
  useEffect(() => {
    importPageCache = {
      state, input, drafts, parseResp, confirmResp, hoveredDraft, pinnedDraft,
      srcScrollTop: sourceScrollRef.current?.scrollTop ?? 0,
      dstScrollTop: draftsScrollRef.current?.scrollTop ?? 0,
      savedAt: Date.now(),
    };
  }, [state, input, drafts, parseResp, confirmResp, hoveredDraft, pinnedDraft]);

  // Persist final scroll positions on unmount (covers "scrolled then
  // navigated away without any other state change" path). Without this,
  // the cache's srcScrollTop/dstScrollTop reflect the last state-change
  // moment, not the actual scroll-when-leaving moment.
  useEffect(() => {
    return () => {
      if (importPageCache) {
        importPageCache = {
          ...importPageCache,
          srcScrollTop: sourceScrollRef.current?.scrollTop ?? importPageCache.srcScrollTop,
          dstScrollTop: draftsScrollRef.current?.scrollTop ?? importPageCache.dstScrollTop,
          savedAt: Date.now(),
        };
      }
    };
...
function renderGroupedDrafts({
  ...
  // 2026-05-06 v15 (用户反馈"卡片顺序与源不一致"根因修复): per-draft 单元 + 连续同 group
  // 合并渲染。
  //
  // 旧 v13: 把整个 group (member_draft_ids 全部) 当一个 unit, anchorLine = min(member lines).
  // 当 group 成员在源里不连续 (被其他 draft 隔开) 时, 整个 group 被锚到第一个成员的行,
  // 后面的成员强行跟过来, 中间的 orphan 被甩到 group 之后 → DOM 顺序违反源行顺序。
  // 例: ANTHROPIC OAuth group = {d11 line11, d14 line17, d15 line22}, orphans d12 line13, d13 line14
  // 旧渲染: [d11 d14 d15] d12 d13      (d14/d15 插到 d12/d13 之前, 错)
  // 新渲染: d11 [group hdr] d12 d13 [d14 d15] [group hdr]  (d14/d15 同组连续, 共享 header)
  //
  // 实现: 每张 draft 自己当一个 unit, anchorLine = 自己的 line_range[0], 带 groupId 元信息。
  // 排序后扫一遍, 把连续 groupId 相同的 draft 合并到同一 EndpointGroupHeader 下。orphan
  // (groupId=null) 各自渲染。这样既保持源行顺序, 又在源内连续时保留 group 视觉聚合。
  const groupMetaById = new Map<string, EndpointGroup>();
  const memberToGroupId = new Map<string, string>();
  for (const g of groups) {
    groupMetaById.set(g.id, g);
    for (const id of g.member_draft_ids) memberToGroupId.set(id, g.id);
  }

  type Unit = {
    draft: DraftRow;
    anchorLine: number;
    groupId: string | null; // null = orphan
  };
  const units: Unit[] = drafts.map((d) => ({
    draft: d,
    anchorLine: d.record.line_range[0],
    groupId: memberToGroupId.get(d.record.id) ?? null,
  }));
  units.sort((a, b) => a.anchorLine - b.anchorLine);

  // Walk and emit either a single orphan or a contiguous-group block.
  type Run =
    | { kind: 'orphan'; unit: Unit }
    | { kind: 'group'; groupId: string; units: Unit[] };
  const runs: Run[] = [];
  for (const u of units) {
    if (u.groupId == null) {
      runs.push({ kind: 'orphan', unit: u });
      continue;
    }
    const tail = runs[runs.length - 1];
    if (tail && tail.kind === 'group' && tail.groupId === u.groupId) {
      tail.units.push(u);
    } else {
      runs.push({ kind: 'group', groupId: u.groupId, units: [u] });
    }
  }
  ...
  return (
    <div className="space-y-3">
      {runs.map((r, ri) => {
        if (r.kind === 'orphan') {
          return <div key={`o-${r.unit.draft.record.id}`} className="space-y-1.5">{renderDraft(r.unit.draft)}</div>;
        }
        const g = groupMetaById.get(r.groupId);
        if (!g) {
          // Defensive: groupId in unit but group meta missing (shouldn't happen).
          return (
            <div key={`g-fallback-${ri}`} className="space-y-1.5">
